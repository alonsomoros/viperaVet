...
import com.alonso.vipera.training.springboot_apirest.controller.CatApiBreedsController;
...
@WebMvcTest(CatApiBreedsController.class)
@AutoConfigureMockMvc
@Import(TestConfig.class)
public class CatApiBreedsControllerTest {
    ...
    @MockBean
    private CatApiBreedsService catApiBreedsService;

    @Test
    @WithMockUser(username = "Alonso")
    void testGetAllBreeds_shouldReturnOkAndBreedsList() throws Exception {
        List<CatApiBreedInDTO> breedsList = List.of(
                new CatApiBreedInDTO("abys", "Abyssinian", "Desc"));

        when(catApiBreedsService.getAllCatBreeds()).thenReturn(breedsList);

        mockMvc.perform(get("/api/cat-breeds")
    ...
    void testSaveAllBreeds_shouldReturnOkAndSavedBreedsList() throws Exception {
        List<BreedOutDTO> breedsList = List.of(new BreedOutDTO(1L, "Abyssinian", null, null));

        when(catApiBreedsService.saveAllCatsBreeds()).thenReturn(breedsList);

        mockMvc.perform(post("/api/cat-breeds/save-all")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
